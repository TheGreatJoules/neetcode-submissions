class Solution {
    ...
    private int dfs(int[][] intervals, int[][] memo, int current, int prev) {
        ...
        int resultPrev = dfs(intervals, memo, current+1, prev);
        if (prev >= 0) {
            if (memo[current][prev] != Integer.MIN_VALUE) {
                return memo[current][prev];
            }
            if (intervals[prev][1] <= intervals[current][0]) {
                return Math.max(resultPrev, 1 + dfs(intervals, memo, current+1, current));
            }
        } else if (prev < 0) {
            int resultCurrent = 1 + dfs(intervals, memo, current+1, current);
            return Math.max(resultPrev, resultCurrent);
        }
        return resultPrev;
    }
}
